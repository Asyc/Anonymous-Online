    ...
    public Client(Socket socket){
        ...
        try {
            ...
            if(packet instanceof HandshakePacket){
                ...
                for(Version version : Server.instance.getAcceptableVersions()){
                    if(version.equals(((HandshakePacket) packet).getVersion())){
                        shouldDisconnect = false;
                    }
                }

                if(shouldDisconnect){
                    this.send(new DisconnectPacket("Server does not support your client version. " +
                            System.lineSeparator() + "Acceptable Versions " + Arrays.toString(Server.instance.getAcceptableVersions())));
                    disconnect();
                }
        ...
        while (socket.isConnected()){
            ...
            try{
                ...
                System.err.println("Could not read sent packet, ignoring.");
                continue;
            }


            Server.EVENT_BUS.post(new PacketReceivedEvent(packet));
        }
    }
